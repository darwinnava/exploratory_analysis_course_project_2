...
author: "Darwin Nava"
date: "April 11, 2021"
file: "Emissions_from_coal_combustion.R"
---
...
## Find the SSC codes where "Coal" appears in the EI.Sector variable. 
## Previous review showed that they all begin with "Fuel Comb" so it is inferred 
## by making this selection the requirement "from coal combustion-related sources" is covered.

scc_coal <- source_pm25[grep("Coal", source_pm25$EI.Sector),1]
pm25_coal <- pm25[pm25$SCC %in% scc_coal,]


by_type_year <- group_by(pm25_coal, year, type)
coal_emissions <- summarize(by_type_year, Emissions=sum(Emissions))
#coal_emissions

g <- qplot(year, Emissions, data=coal_emissions, facets = .~type)
g + labs(title = "Emission of PM2.5 in USA from coal combustion-related source") + labs(x="The total decrease in PM2.5 emissions is observed in in all sources", y=expression("Emission in tons of  "*PM[2.5]))     

## generating png file and closing device
#dev.copy(png, file="Emission_pm25_coal_USA_by_year_by_type.png", width=480, height=480)
