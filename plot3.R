...
author: "Darwin Nava"
date: "April 11, 2021"
file: "Emission_pm25_Baltimore_by_year_by_type.R"
---
...
by_type_year <- group_by(pm25_baltimore, type, year)
total_emissions <- summarise(by_type_year, Emissions=sum(Emissions))

g <- qplot(year, Emissions, data=total_emissions, facets = .~type)
g + labs(title = "Emission of PM2.5 in Baltimore by source type") + labs(x="The total decrease in PM2.5 emissions is observed in in all sources except point source.", y=expression("Emission in tons of  "*PM[2.5]))     

## generating png file and closing device
#dev.copy(png, file="Emission_pm25_Baltimore_by_year_by_type.png", width=480, height=480)
