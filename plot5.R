...
author: "Darwin Nava"
date: "April 11, 2021"
file: "Emissions_from_motor_vehicles.Baltimore.R"
---
...
pm25_motor_baltimore <- subset(pm25_motor, fips=="24510")
by_type_year <- group_by(pm25_motor_baltimore, type, year)
motor_v_emissions <- summarize(by_type_year, Emissions = sum(Emissions))
#road_vehicleemissions

g <- qplot(year, Emissions, data=motor_v_emissions, facets = .~type)
g + labs(title = "Emission of PM2.5 in Baltimore from motor/road vehicles sources") + labs(x="The total decrease in PM2.5 emissions is observed in in on-road source. ", y=expression("Emission in tons of  "*PM[2.5]))     

## generating png file and closing device
#dev.copy(png, file="Emission_pm25_motor_vehicle_Baltimore_by_year_by_type.png", width=480, height=480)
